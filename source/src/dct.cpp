...
void_t izigzag_4x4_2d(int16_t* raw_input_data, int** output_data, int** Quantum_2d) {
	...
	for (i = 0; i < (N * N); i++) {
		...
		//row = zigzag_8x8[i].row;
		//col = zigzag_8x8[i].col;
		output_data[row][col] = raw_input_data[i] *
			Quantum_2d[row][col];
	}
}
...
void_t get_dct16_hevc(int* dst, int* src, int shift, int line) {
	...
	for (j = 0; j < line; j++) {
		...
		dst[0] = (int16_t)((g_aiT16[TRANSFORM_FORWARD][0][0] * EEE[0] + g_aiT16[TRANSFORM_FORWARD][0][1] * EEE[1] +
			add) >> shift);
		dst[8 * line] = (int16_t)((g_aiT16[TRANSFORM_FORWARD][8][0] * EEE[0] + g_aiT16[TRANSFORM_FORWARD][8][1] * EEE[1] +
			add) >> shift);
		dst[4 * line] = (int16_t)((g_aiT16[TRANSFORM_FORWARD][4][0] * EEO[0] + g_aiT16[TRANSFORM_FORWARD][4][1] * EEO[1] +
			add) >> shift);
		dst[12 * line] = (int16_t)((g_aiT16[TRANSFORM_FORWARD][12][0] * EEO[0] + g_aiT16[TRANSFORM_FORWARD][12][1] * EEO[1] +
			add) >> shift);

		for (k = 2; k < 16; k += 4) {
			dst[k * line] = (int16_t)((g_aiT16[TRANSFORM_FORWARD][k][0] * EO[0] + g_aiT16[TRANSFORM_FORWARD][k][1] * EO[1] +
				g_aiT16[TRANSFORM_FORWARD][k][2] * EO[2] + g_aiT16[TRANSFORM_FORWARD][k][3] * EO[3] +
				add) >> shift);
		}

		for (k = 1; k < 16; k += 2) {
			dst[k * line] = (int16_t)((g_aiT16[TRANSFORM_FORWARD][k][0] * O[0] + g_aiT16[TRANSFORM_FORWARD][k][1] * O[1] +
				...
				g_aiT16[TRANSFORM_FORWARD][k][6] * O[6] + g_aiT16[TRANSFORM_FORWARD][k][7] * O[7] +
				add) >> shift);
		}
